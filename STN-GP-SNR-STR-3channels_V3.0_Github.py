...
for thisTrial in range(0, numtrials):
    ...
    strd1_cell_params = {'a': strd1_a, 'b': strd1_b, 'c': strd1_c, 'd': strd1_d,
                           'v_init': strd1_v_init, 'u_init': strd1_u_init,
                           'tau_syn_E': tau_ampa, 'tau_syn_I': tau_gabaa,
                           'i_offset': current_bias_str,
                           'e_rev_E': E_ampa, 'e_rev_I': E_gabaa,
                           }

    strd2_cell_params = {'a': strd2_a, 'b': strd2_b, 'c': strd2_c, 'd': strd2_d,
                           'v_init': strd2_v_init, 'u_init': strd2_u_init,
                           'tau_syn_E': tau_ampa, 'tau_syn_I': tau_gabaa,
                           'i_offset': current_bias_str,
                           'e_rev_E': E_ampa, 'e_rev_I': E_gabaa,
                           }
    ...
    fsi_cell_params = {'a': fsi_a, 'b': fsi_b, 'c': fsi_c, 'd': fsi_d,
                           'v_init': fsi_v_init, 'u_init': fsi_u_init,
                           'tau_syn_E': tau_ampa, 'tau_syn_I': tau_gabaa,
    ...
    gpe_cell_params = {'a': gpe_a, 'b': gpe_b, 'c': gpe_c, 'd': gpe_d,
                       'v_init': gpe_v_init, 'u_init': gpe_u_init,
                       'tau_syn_E': tau_ampa, 'tau_syn_I': tau_gabaa,
    ...
    snr_cell_params = {'a': snr_a, 'b': snr_b, 'c': snr_c, 'd': snr_d,
                       'v_init': snr_v_init, 'u_init': snr_u_init,
                       'tau_syn_E': tau_ampa, 'tau_syn_I': tau_gabaa,
    ...
    ''' THE SECOND CHANNEL'''
    strd1_pop2 = p.Population(numCellsPerCol_STR, p.IZK_cond_exp, strd1_cell_params, label='strd1_pop2')
    strd2_pop2 = p.Population(numCellsPerCol_STR, p.IZK_cond_exp, strd2_cell_params, label='strd2_pop2')
    gpe_pop2 = p.Population(numCellsPerCol_GPe, p.IZK_cond_exp, gpe_cell_params, label='gpe_pop2')
    snr_pop2 = p.Population(numCellsPerCol_SNR, p.IZK_cond_exp, snr_cell_params, label='snr_pop2')
    stn_pop2 = p.Population(numCellsPerCol_STN, p.IZK_cond_exp, stn_cell_params, label='stn_pop2')
    fsi1_pop2 = p.Population(numCellsPerCol_FSI, p.IZK_cond_exp, fsi_cell_params, label='fsi1_pop2')


    ''' THE THIRD CHANNEL'''
    strd1_pop3 = p.Population(numCellsPerCol_STR, p.IZK_cond_exp, strd1_cell_params, label='strd1_pop3')
    strd2_pop3 = p.Population(numCellsPerCol_STR, p.IZK_cond_exp, strd2_cell_params, label='strd2_pop3')
    gpe_pop3 = p.Population(numCellsPerCol_GPe, p.IZK_cond_exp, gpe_cell_params, label='gpe_pop3')
    snr_pop3 = p.Population(numCellsPerCol_SNR, p.IZK_cond_exp, snr_cell_params, label='snr_pop3')
    stn_pop3 = p.Population(numCellsPerCol_STN, p.IZK_cond_exp, stn_cell_params, label='stn_pop3')
    fsi1_pop3 = p.Population(numCellsPerCol_FSI, p.IZK_cond_exp, fsi_cell_params, label='fsi1_pop3')
    ...
    for count1 in range(0, 4):
        if count1 < 3:
            p.Projection(spike_source_Poisson_base1, poplist_ch1[count1],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2str, weights=g_pop[count1], delays=distr_pop[count1]),
                         target='excitatory')
        else:
            p.Projection(spike_source_Poisson_base2, poplist_ch1[count1],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2stn, weights=g_pop[count1], delays=distr_pop[count1]),
                         target='excitatory')
    ...
    for count2 in range(0, 4):
        if count2 < 3:
            p.Projection(spike_source_Poisson_base1, poplist_ch2[count2],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2str, weights=g_pop[count2], delays=distr_pop[count2]),
                         target='excitatory')
        else:
            p.Projection(spike_source_Poisson_base2, poplist_ch2[count2],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2stn, weights=g_pop[count2], delays=distr_pop[count2]),
                         target='excitatory')
    ...
    for count3 in range(0, 4):
        if count3 < 3:
            p.Projection(spike_source_Poisson_base1, poplist_ch3[count3],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2str, weights=g_pop[count3], delays=distr_pop[count3]),
                         target='excitatory')
        else:
            p.Projection(spike_source_Poisson_base2, poplist_ch3[count3],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2stn, weights=g_pop[count3], delays=distr_pop[count3]),
                         target='excitatory')
    ...
    for count00 in range(0, 4):
        if count00 < 3:
            p.Projection(spike_source_Poisson_first1, poplist_ch1[count00],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2str, weights=g_pop[count00], delays=distr_pop[count00]),
                         target='excitatory')
        else:
            p.Projection(spike_source_Poisson_first2, poplist_ch2[count00],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2stn, weights=g_pop[count00], delays=distr_pop[count00]),
                         target='excitatory')
    ...
    for count01 in range(0, 4):
        if count01 < 3:
            p.Projection(spike_source_Poisson_second1, poplist_ch2[count01],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2str, weights=g_pop[count01], delays=distr_pop[count01]),
                         target='excitatory')
        else:
            p.Projection(spike_source_Poisson_second2, poplist_ch2[count01],
                         p.FixedProbabilityConnector(p_connect=pconn_cort2stn, weights=g_pop[count01], delays=distr_pop[count01]),
                         target='excitatory')
    ...
    p.Projection(strd1_pop1, snr_pop1,
                 p.FixedProbabilityConnector(p_connect=0.15, weights = g_strd12snr, delays=distr_strd12snr),
                 target='inhibitory')
    p.Projection(strd2_pop1, gpe_pop1,
                 p.FixedProbabilityConnector(p_connect=0.15, weights = g_strd22gpe, delays=distr_strd22gpe),
                 target='inhibitory')
    p.Projection(strd1_pop1, strd1_pop1,
                 p.FixedProbabilityConnector(p_connect=0.1, weights = g_str2str, delays=distr_str2str),
                 target='inhibitory')
    ...
    p.Projection(gpe_pop1, stn_pop1,
                 p.FixedProbabilityConnector(p_connect=0.25, weights=g_gpe2stn, delays=distr_gpe2stn),
                 target='inhibitory')
    p.Projection(gpe_pop1, gpe_pop1,
                 p.FixedProbabilityConnector(p_connect=0.25, weights=g_gpe2gpe, delays=distr_gpe2gpe),
                 target='inhibitory')
    p.Projection(gpe_pop1, snr_pop1,
                 p.FixedProbabilityConnector(p_connect=0.25, weights=g_gpe2snr, delays=distr_gpe2snr),
                 target='inhibitory')
    p.Projection(gpe_pop1, fsi1_pop1,
                 p.FixedProbabilityConnector(p_connect=0.05, weights=g_gpe2fsi, delays=distr_gpe2fsi),
                 target='inhibitory')
    ...
    p.Projection(snr_pop1, snr_pop1,
                 p.FixedProbabilityConnector(p_connect=0.25, weights=g_snr2snr, delays=distr_snr2snr),
                 target='inhibitory')
    ...
    p.Projection(stn_pop1, gpe_pop1,
                 p.FixedProbabilityConnector(p_connect=p_conn_diffuse, weights=g_stn2gpe_diffuse, delays=distr_stn2gpe),
                 target='excitatory')
    p.Projection(stn_pop1, snr_pop1,
                 p.FixedProbabilityConnector(p_connect=p_conn_diffuse, weights=g_stn2snr_diffuse, delays=distr_stn2snr),
                 target='excitatory')

    '''projections from chanel 1 to chanel 2'''
    p.Projection(stn_pop1, gpe_pop2,
                 p.FixedProbabilityConnector(p_connect=p_conn_diffuse, weights=g_stn2gpe_diffuse, delays=distr_stn2gpe_diffuse),
                 target='excitatory')
    p.Projection(stn_pop1, snr_pop2,
                 p.FixedProbabilityConnector(p_connect=p_conn_diffuse, weights=g_stn2snr_diffuse, delays=distr_stn2snr_diffuse),
                 target='excitatory')
    ...
    # spike_source_Poisson_base1.record()
    # strd1_pop1.record()
    # strd2_pop1.record()
    fsi1_pop1.record()
    # fsi2_pop1.record()
    ...
    # spike_source_Poisson_raster1 = np.asarray(spike_source_Poisson_base1.getSpikes())
    #
    # strd1_spike_raster1 = np.asarray(strd1_pop1.getSpikes())
    # strd2_spike_raster1 = np.asarray(strd2_pop1.getSpikes())
    stn_spike_raster1 = np.asarray(stn_pop1.getSpikes())
    gpe_spike_raster1 = np.asarray(gpe_pop1.getSpikes())
    snr_spike_raster1 = np.asarray(snr_pop1.getSpikes())
    fsi1_spike_raster1 = np.asarray(fsi1_pop1.getSpikes())


    # strd1_spike_raster2 = np.asarray(strd1_pop2.getSpikes())
    # strd2_spike_raster2 = np.asarray(strd2_pop2.getSpikes())
    stn_spike_raster2 = np.asarray(stn_pop2.getSpikes())
    gpe_spike_raster2 = np.asarray(gpe_pop2.getSpikes())
    snr_spike_raster2 = np.asarray(snr_pop2.getSpikes())
    fsi1_spike_raster2 = np.asarray(fsi1_pop2.getSpikes())

    # strd1_spike_raster3 = np.asarray(strd1_pop3.getSpikes())
    # strd2_spike_raster3 = np.asarray(strd2_pop3.getSpikes())
    stn_spike_raster3 = np.asarray(stn_pop3.getSpikes())
    gpe_spike_raster3 = np.asarray(gpe_pop3.getSpikes())
    snr_spike_raster3 = np.asarray(snr_pop3.getSpikes())
    fsi1_spike_raster3 = np.asarray(fsi1_pop3.getSpikes())
